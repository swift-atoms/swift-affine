...
//
// File covers the previous-generation continuous-affine + linear-algebra
// operator-integration surface (`Affine<T,_>.X ± Linear<T,_>.Dx` etc.). Out of
// scope for the 0.1.0 discrete surface. The current cross-cutting operator
// surface lives at `OrdinalAffineArithmeticTests.swift` (Position ± Vector,
// Position − Position) and `TaggedAffineOffsetTests.swift` (Tagged-side
// arithmetic + Vector ↔ Cardinal cross-type comparisons).
