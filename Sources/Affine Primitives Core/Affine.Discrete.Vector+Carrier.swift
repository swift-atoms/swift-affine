...
// Affine.Discrete.Vector conforms to Carrier as a trivial-self-carrier;
// per-type accessor + arithmetic + constants live as constrained
// extensions on `Carrier where Underlying == Affine.Discrete.Vector`.
...
extension Carrier where Underlying == Affine.Discrete.Vector {
    /// The underlying vector value.
    ///
    /// Synonym for `underlying` — preserves the per-type accessor name
    /// established by the (now-removed)
    /// `Affine.Discrete.Vector.\`Protocol\``.
    @inlinable
...
/// Negates a vector.
@inlinable
public prefix func - <V: Carrier<Affine.Discrete.Vector>>(v: V) -> V {
